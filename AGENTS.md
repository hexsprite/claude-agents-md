...
- `tests/` — Bun PTY tests. `scan-script.test.ts` is pure script unit tests (no API);
  `session-start.test.ts` / `file-changed.test.ts` spawn a real Claude session (~60s, burns credits).
